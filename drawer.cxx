...
void dr::Drawer::drawNonogram() const {
  ...
  glPushMatrix();
    ...
    drawNonogramColumns();
    drawNonogramMatrix();
    drawResult(0.5f, -0.5f, nonogram->isCorrect(), 2.05f);
  glPopMatrix();
}

void dr::Drawer::drawNonogramRows() const {
  for(int i = 0, y = -2; i < nonogram->getRows(); i++, y--) {
    drawResult(0, y, nonogram->isCorrect(ROW, i));
...
void dr::Drawer::drawNonogramColumns() const {
  for(int j = 0, x = 2; j < nonogram->getColumns(); j++, x++) {
    drawResult(x, 0, nonogram->isCorrect(COL, j));
